...
async function proxy(url: string, method: string, body?: unknown) {
  try {
    ...
    console.error('project-folders/[id] proxy error:', error);
...
export async function PATCH(req: Request, { params }: Params) {
  ...
  return proxy(`${API_URL}/project-folders/${id}`, 'PATCH', await req.json());
}

export async function DELETE(_req: Request, { params }: Params) {
  const { id } = await params;
  return proxy(`${API_URL}/project-folders/${id}`, 'DELETE');
}
