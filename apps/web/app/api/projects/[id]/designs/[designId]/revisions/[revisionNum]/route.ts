...
type Params = { params: Promise<{ id: string; designId: string; revisionNum: string }> };
...
async function proxy(url: string, method: string, body?: unknown) {
  try {
    ...
    console.error('projects/[id]/designs/[designId]/revisions/[revisionNum] proxy error:', error);
    return NextResponse.json({ error: 'Failed to fetch from server' }, { status: 500 });
  }
}

export async function GET(_req: Request, { params }: Params) {
  const { id, designId, revisionNum } = await params;
  return proxy(`${API_URL}/projects/${id}/designs/${designId}/revisions/${revisionNum}`, 'GET');
}
