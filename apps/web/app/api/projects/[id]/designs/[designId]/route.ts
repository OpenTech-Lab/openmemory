...
async function proxy(url: string, method: string, body?: unknown) {
  try {
    ...
    console.error('projects/[id]/designs/[designId] proxy error:', error);
...
export async function PUT(req: Request, { params }: Params) {
  ...
  return proxy(`${API_URL}/projects/${id}/designs/${designId}`, 'PUT', body);
}

export async function DELETE(_req: Request, { params }: Params) {
  const { id, designId } = await params;
  return proxy(`${API_URL}/projects/${id}/designs/${designId}`, 'DELETE');
}
