...
export async function POST(req: Request, { params }: Params) {
  ...
  try {
    const response = await fetch(`${API_URL}/projects/${id}/sync`, {
      method: 'POST',
      headers: {
    ...
    console.error('projects/[id]/sync proxy error:', error);
    return NextResponse.json({ error: 'Failed to sync project files' }, { status: 500 });
  }
}
