...
async function proxy(url: string, method: string, body?: unknown) {
  try {
    ...
    console.error('projects/[id]/lessons/[lessonId] proxy error:', error);
...
export async function PUT(req: Request, { params }: Params) {
  ...
  return proxy(`${API_URL}/projects/${id}/lessons/${lessonId}`, 'PUT', body);
}

export async function DELETE(_req: Request, { params }: Params) {
  const { id, lessonId } = await params;
  return proxy(`${API_URL}/projects/${id}/lessons/${lessonId}`, 'DELETE');
}
