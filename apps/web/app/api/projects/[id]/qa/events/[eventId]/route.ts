...
async function proxy(url: string, method: string, body?: unknown) {
  try {
    ...
    console.error('projects/[id]/qa/events/[eventId] proxy error:', error);
...
export async function PATCH(req: Request, { params }: Params) {
  ...
  return proxy(`${API_URL}/projects/${id}/qa/events/${eventId}`, 'PATCH', body);
}

export async function DELETE(_req: Request, { params }: Params) {
  const { id, eventId } = await params;
  return proxy(`${API_URL}/projects/${id}/qa/events/${eventId}`, 'DELETE');
}
