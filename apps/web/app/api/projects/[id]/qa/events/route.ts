...
async function proxy(url: string, method: string, body?: unknown) {
  try {
    ...
    console.error('projects/[id]/qa/events proxy error:', error);
...
export async function GET(_req: Request, { params }: Params) {
  ...
  return proxy(`${API_URL}/projects/${id}/qa/events`, 'GET');
}

export async function POST(req: Request, { params }: Params) {
  const { id } = await params;
  const body = await req.json();
  return proxy(`${API_URL}/projects/${id}/qa/events`, 'POST', body);
}
