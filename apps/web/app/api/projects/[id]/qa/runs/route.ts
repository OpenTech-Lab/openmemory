...
async function proxy(url: string, method: string, body?: unknown) {
  try {
    ...
    console.error('projects/[id]/qa/runs proxy error:', error);
...
export async function GET(req: NextRequest, { params }: Params) {
  ...
  return proxy(`${API_URL}/projects/${id}/qa/runs${search}`, 'GET');
}

export async function POST(req: Request, { params }: Params) {
  const { id } = await params;
  const body = await req.json();
  return proxy(`${API_URL}/projects/${id}/qa/runs`, 'POST', body);
}
