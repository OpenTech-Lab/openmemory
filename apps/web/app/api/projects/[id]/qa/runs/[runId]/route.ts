...
async function proxy(url: string, method: string, body?: unknown) {
  try {
    ...
    console.error('projects/[id]/qa/runs/[runId] proxy error:', error);
...
export async function GET(_req: Request, { params }: Params) {
  ...
  return proxy(`${API_URL}/projects/${id}/qa/runs/${runId}`, 'GET');
}

export async function PATCH(req: Request, { params }: Params) {
  const { id, runId } = await params;
  const body = await req.json();
  return proxy(`${API_URL}/projects/${id}/qa/runs/${runId}`, 'PATCH', body);
}

export async function DELETE(_req: Request, { params }: Params) {
  const { id, runId } = await params;
  return proxy(`${API_URL}/projects/${id}/qa/runs/${runId}`, 'DELETE');
}
