...
async function proxy(url: string, method: string, body?: unknown) {
  try {
    ...
    console.error('projects/[id]/qa/evidence/[evidenceId] proxy error:', error);
...
export async function PATCH(req: Request, { params }: Params) {
  ...
  return proxy(`${API_URL}/projects/${id}/qa/evidence/${evidenceId}`, 'PATCH', body);
}

export async function DELETE(_req: Request, { params }: Params) {
  const { id, evidenceId } = await params;
  return proxy(`${API_URL}/projects/${id}/qa/evidence/${evidenceId}`, 'DELETE');
}
