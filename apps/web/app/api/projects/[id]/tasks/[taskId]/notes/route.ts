...
async function proxy(url: string, method: string, body?: unknown) {
  try {
    ...
    console.error('task notes proxy error:', error);
...
export async function GET(_req: Request, { params }: Params) {
  ...
  return proxy(`${API_URL}/projects/${id}/tasks/${taskId}/notes`, 'GET');
}

export async function POST(req: Request, { params }: Params) {
  const { id, taskId } = await params;
  const body = await req.json();
  return proxy(`${API_URL}/projects/${id}/tasks/${taskId}/notes`, 'POST', body);
}
