...
export function ProjectDesignPanel({ projectId, projectPath }: ProjectDesignPanelProps) {
  ...
  const openCreate = () => {
    setEditDesign(null);
    setEditForm({ ...EMPTY_EDIT_FORM, source: STARTER_TEMPLATES[EMPTY_EDIT_FORM.kind as DesignKind] });
    setAiPrompt('');
    setIsEditOpen(true);
  };
  ...
  const handleGenerateDiagram = async () => {
    ...
    try {
      const res = await fetch('/api/memory', {
        ...
        body: JSON.stringify({ type: 'ai.design_diagram', prompt, kind: editForm.kind }),
      });
      const data = await res.json();
  ...
  // Auto-fill the starter template when the kind changes on a NEW doc whose source is still
  // untouched (empty, or exactly a previous starter template) — never clobber real edits.
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Mermaid diagrams for this project&apos;s UI, structure, workflows, or story.
          </p>
        </div>
