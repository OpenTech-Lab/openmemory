...
struct RawLineItem {
    ...
    usage: Option<String>,
    #[serde(default)]
    monthly_cost_usd: Option<serde_json::Value>,
    #[serde(default)]
    notes: Option<String>,
}
...
fn user_message(
    ...
    format!(
        "<design title=\"{}\" kind=\"{}\">\n{}\n</design>\n<conditions>\n{}\n</conditions>",
        llm::truncate(design_title, 200),
        llm::truncate(design_kind, 80),
        llm::truncate(design_source, MAX_DESIGN_LEN),
        llm::truncate(conditions, MAX_CONDITIONS_LEN),
    )
}
...
pub fn parse_estimate(raw_json: &str) -> BudgetEstimate {
        ...
        .filter_map(|item| {
            ...
            Some(BudgetLineItem {
                ...
                usage: llm::truncate(
                    &item
                        .usage
                        .and_then(|v| llm::safe_str(Some(v)))
                        .unwrap_or_default(),
                    300,
                ),
    ...
    BudgetEstimate {
        line_items,
        confidence: if ["low", "medium", "high"].contains(&confidence.as_str()) {
            confidence
        } else {
            "low".into()
        },
        pricing_basis: raw
            .pricing_basis
            .and_then(|v| llm::safe_str(Some(v)))
            .map(|v| llm::truncate(&v, 1000))
            .unwrap_or_else(|| {
                "Approximate monthly public pricing; verify with the provider calculator.".into()
            }),
    }
}
...
mod tests {
    ...
    fn parses_and_converts_dollars_to_cents() {
        let estimate = parse_estimate(
            r#"{"line_items":[{"service":"AWS Lambda","usage":"1m requests","monthly_cost_usd":12.34}],"confidence":"medium","pricing_basis":"us-east-1"}"#,
        );
